...
**Superseded-Note:** The Operator-Block-Details above are the PRE-DECISION state and are retained as the record of the decision point, not as current fact. The operator chose [A]; verification then found [A] was ALREADY MERGED at 78b2c0ac (@modelcontextprotocol/sdk ^0.5.0 -> ^1.24.0, installed 1.30.0; ws 8.21.1), so all three advisories are out of range and npm audit reports 0 vulnerabilities, confirmed against a golden-bad control that reproduced all three on the old pins. The "~79 packages / ~3600 tracked files" objection to [A] no longer applies either: 0a48f8c2 untracked 104MB of node_modules. The sentence "is NOT closed and needs an owner decision" was true when written and is not true now. UNVERIFIED (11.4.6): container-layer runtime evidence — the tests ran the built dist/ on the host, not the image. STILL OPEN as hygiene, not security: ws and the SDK are unused dead dependencies; removing them is a separate 11.4.122 + 11.4.124 item.
**Severity:** High

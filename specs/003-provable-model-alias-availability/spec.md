...
- **Refined baseline for the 19 unusable aliases (measured 2026-09-08, read-only).** They are NOT merely missing a configuration file. 18 of the 19 correspond to providers the system reports as *orphaned* — present as a record, absent from the current resolved set — and 1 is usable-but-unrendered. Every one of the 29 key variables is present in the environment, and the model catalogue is healthy and names them, so this is NOT a credentials problem, which was the first and wrong reading. The 20 orphaned records split into two classes needing different remedies: **9 carry no pin under their own identifier** (`nvidia2-4`, `openrouter2-5`, `sarvam`, `zai`) and share a key variable with siblings, so catalogue matching — which yields exactly one record per key variable — can never reach them; adding a pin is the candidate repair. **11 are pinned and orphaned anyway**, so pinning is necessary but not sufficient and their cause is UNDETERMINED. Separately, `huggingface`'s key variable is absent from the catalogue entirely, making it the most likely FR-020 "genuinely un-repairable, returns as a fresh decision" case. Determining the second class's cause requires running the resolver, which was deliberately not done while that codebase was being edited.
